...
public class ArticleCategoryService extends IdEntityService<ArticleCategory> {
	...
	public List<ArticleCategory> findParents(ArticleCategory articleCategory) {
		...
		return dao().query(getEntityClass(), Cnd.where("id", "in", articleCategory.getTreePaths()).limit(1,10).asc("grade"));
	}

	public List<ArticleCategory> findChildren(ArticleCategory articleCategory) {
		if (articleCategory != null) {
			return dao().query(getEntityClass(), Cnd.where("treePath", "like", "%," + articleCategory.getId() + "," + "%").limit(1,10).asc("order"));
		} else {
			return dao().query(getEntityClass(), Cnd.NEW().limit(1, 10).asc("order"));
		}
	}
}
