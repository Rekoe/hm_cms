...
public class HtmlParserImpl implements HtmlParser {
	...
	public static void main(String[] args) {
		String orginHtml = Http.get("http://news.163.com/domestic/").getContent();
		HtmlParserImpl htmlParser = new HtmlParserImpl(null);
		Parser parser = new Parser();
		Map<String, String> fetchAreaTagMap = new HashMap<>();
		fetchAreaTagMap.put("class", "list-item clearfix");
		try {
			orginHtml = htmlParser.filterRequire(fetchAreaTagMap, parser, orginHtml);
			System.out.println(orginHtml);
		} catch (ParserException e) {
			e.printStackTrace();
		}
